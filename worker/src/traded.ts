...
export const handleTradedTokenEvent = async (aptos: Aptos): Promise<void> => {
  await db.begin(async tx => {
    ...
    log.debug({ idx }, '[Traded]: processing events');
    ...
    if (events.length === 0) {
      log.debug('[Traded]: skips as no more events to process');
      return;
    }
    ...
    for (const t of updatingTokenData) {
      ...
      const registerHolderJob = tx`insert into worker_holders(token_addr) values (${t.tokenAddr}) on conflict(token_addr) do nothing`;
