...
export const handlePoolCompletedEvent = async (aptos: Aptos) => {
  await db.begin(async tx => {
    ...
    log.debug({ idx }, '[PoolCompleted]: processing events');
    ...
    if (events.length === 0) {
      log.debug('[PoolCompleted]: skips as no more events to process');
      return;
    }
    ...
    for (const data of updatingTokenData) {
      promises.push(syncHolder(aptos, tx, data.addr));
      const updatingTokenPm =
        tx`update tokens set ${tx(data, tokenUpdatedColumns)} where addr = ${data.addr}`.then(
          ({ count }) => {
            // will retry later
            let result: unknown;
            if (count <= 0)
              throw new WorkerError(
                `[${idx.event}]: Updating token(${data.addr}) not found.`,
                { ignore: true }
              );
            return result;
          }
        );
      promises.push(updatingTokenPm);
    }
